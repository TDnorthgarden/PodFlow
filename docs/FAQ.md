...
4. **配置检查**:
   ```bash
   nuts-observer --config /etc/nuts/config.yaml --dry-run
   # 验证配置文件格式
   ```
...
2. **过滤目标**:
   ```bash
   # 只监控关键命名空间
   nuts-observer watch --namespace production --exclude-namespace test
   ```
...
datasources:
  ...
  bpftrace:
    enabled: true
    scripts_path: /opt/nuts/bpftrace
...
```bash
# 命令行指定
nuts-observer --server http://prod-api:8080

# 环境变量
export NUTS_SERVER=http://prod-api:8080
...
1. **启用内存监控**:
   ```bash
   nuts-observer watch --pod-uid abc123 --evidence-types memory,oom_events --interval 30
   ```
...
2. **网络监控**:
   ```bash
   nuts-observer watch --pod-uid abc123 --evidence-types network --metrics latency_p99,packet_loss
   ```
...
1. **创建bpftrace脚本**:
   ```bash
   mkdir /opt/nuts/bpftrace/custom_probe
   cd /opt/nuts/bpftrace/custom_probe
   # 创建新的探针脚本
   ```
...
2. **注册探针**:
   ```bash
   # 在配置文件中注册
   echo "probes:" >> /etc/nuts/config.yaml
   echo "  - name: custom_probe" >> /etc/nuts/config.yaml
   echo "    path: /opt/nuts/bpftrace/custom_probe/probe.bt" >> /etc/nuts/config.yaml
   ```

3. **重载配置**:
   ```bash
   sudo systemctl reload nuts-observer
   ```

### Q: 如何自定义诊断规则？
A: 自定义规则开发：

1. **规则文件位置**: `/etc/nuts/rules/`
2. **规则格式**:
   ```yaml
   rules:
...
3. **热加载**:
   ```bash
   # 无需重启，自动加载新规则
   nuts-observer config reload-rules
   ```
...
2. **社区支持**:
   ...
   - GitHub Discussions: https://github.com/your-org/nuts-observer/discussions
